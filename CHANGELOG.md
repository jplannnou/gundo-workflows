...
### Fixed
- `reusable-deploy-cloudrun.yml`: point the Feedback Hub reporter default at the live `gundo-feedback-api` service. The previous `gundo-content-engine` service was retired, so every lifecycle event returned HTTP 404 while deploys continued without DORA telemetry.
- All reusable workflows now use `google-github-actions/setup-gcloud@v3`, whose action runtime is Node 24. This removes the Node 20 deprecation warning before GitHub disables that runtime.
- `reusable-deploy-cloudrun.yml`: **barrido de tags `canary-*` huérfanos de runs anteriores**, más `timeout-minutes: 30` en el step de despliegue. Cierra el hueco que dejaban abiertos los tres arreglos previos de limpieza (`5c9cfeed` untag en promote/rollback, `815bacaa` limpieza al cancelar, `e4f545e` tag calculado antes de gcloud): **los tres son AUTOLIMPIEZA** — cada run retira solo su propio `canary-<sha>` — así que ninguno cubre los caminos en los que el job deja de existir sin ejecutar nada, ni siquiera los `always()` (runner self-hosted apagado o reiniciado, job agotado por timeout mientras `gcloud run deploy` está bloqueado esperando al plano de control). Y como ningún run miraba los tags de otros runs, **un solo escape se volvía permanente**. Medido en vivo el 15-ago-2026: 29 tags huérfanos supervivientes — 17 en `genie-ui` (2-7 de mayo, anteriores a toda limpieza), 11 en `genie-api` (10-11 de julio, con revisiones que tardaron entre 3h30m y 9h52m en aprovisionar: `Deploying revision succeeded in 9h52m14.38s`) y 1 en `gundo-ecommerce-ui` (29 de julio). Ahora cada despliegue barre primero lo que quedó colgado, así que un escape se cura en el despliegue siguiente en vez de quedarse para siempre. El barrido nunca hace fallar el deploy (`continue-on-error`) y solo retira tags al 0% de tráfico y con más de 60 minutos de vida, de modo que no puede tocar un canary en vuelo.
- `reusable-deploy-cloudrun.yml`: pass deploy flags as a bash array (one per line via `$RUNNER_TEMP/deploy-flags.txt` + `mapfile -t flags` + `"${flags[@]}"`) instead of as a single string interpolated raw via `${{ steps.flags.outputs.flags }}`. The old approach let bash re-parse `--set-env-vars` values: any `|` (pipe) or `||` (logical OR) in a value was treated as a shell operator. Engine PR #48 hit this on `SELLER_NOTIFICATION_PHONES=phone1|phone2` — the deploy step exited with code 127 ("command not found") because bash truncated the gcloud command at the first `|`, executed the suffix as a separate command, and the `--tag=canary-X` flag never reached gcloud (revision was created, but with no tag → unable to capture by tag → step failed). Same trap was waiting for any consumer using `env-vars-separator: '||'` (Feedback Hub for `ALLOWED_ORIGINS`). Array expansion preserves each token verbatim, so values may now contain any shell metacharacter without escaping. Discovered 2026-05-08 on Engine migration; Radar pilot worked only because its env values are pipe-free.
- `reusable-deploy-cloudrun.yml`: resolve the new canary revision by tag (jq filter on `status.traffic[].tag`) instead of `status.traffic[0].revisionName`. After `gcloud run deploy --no-traffic --tag=canary-X`, the first traffic entry is still the **previous** revision serving 100%, not the newly created canary. The old code captured the wrong revision, so the subsequent promote step ran `--to-revisions=<previous>=100 --remove-tags=canary-X` — effectively a no-op that stripped the canary tag and left the new revision orphaned at 0%. Discovered 2026-05-08 on the Radar pilot migration (jplannnou/Gundo_Radar#42) where the deploy "succeeded" but the new code never received traffic. The new lookup mirrors what the Radar/Engine/Feedback inline `ci.yml` jobs always did. Includes a 5-retry loop with 3s backoff so eventual consistency on `status.traffic` doesn't cause a false negative on fast deploys.
